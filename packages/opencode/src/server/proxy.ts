...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-nexuscode-directory")
  out.delete("x-nexuscode-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-nexuscode-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-nexuscode-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
