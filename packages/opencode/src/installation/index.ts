...
const PACKAGE_NAME = "@nexus-code/cli"
    ...
    Effect.gen(function* () {
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          const response = yield* httpOk.execute(HttpClientRequest.get("https://nexusrouter.net/install"))
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          const headers = yield* text([
            "curl",
            "-sI",
            "https://github.com/nexusrouters/nexus-code/releases/latest",
          ])
