...
const MODEL_PARAM_DESCRIPTION =
  "(optional) Model for this subagent: a model group name (e.g. ultra/standard/lite) or a literal provider/model (e.g. nexus-v2.5-pro). Overrides the agent's configured model; defaults to the agent's model, else the parent's. If no model_groups are configured, the tier names resolve to the default model."
...
export function recoverActorArgs(rawArgs: unknown): ActorShellArgs | undefined {
  ...
  if (typeof subagent_type === "string" && typeof description === "string" && typeof prompt === "string") {
    ...
    // the bare shape nexus emits is the 3 required fields, rarely with extras. When
  ...
  Effect.gen(function* () {
    ...
    return Effect.fn("ActorTool.init")(function* () {
      ...
      const parameters = z.strictObject({
        ...
        // (notably nexus-v2.5-pro) stringify the whole envelope
