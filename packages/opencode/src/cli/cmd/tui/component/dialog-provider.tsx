...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@nexus-code/sdk/v2"
...
export async function runCustomProviderWizard(opts: {
  ...
  const providerIDRaw = await step(1, 6, "Provider id", "e.g. nexusrouter")
  if (providerIDRaw === null) return
  const providerID = providerIDRaw.trim()
  if (!providerID) return

  const nameRaw = await step(2, 6, "Display name", "e.g. Nexus Router", providerID)
