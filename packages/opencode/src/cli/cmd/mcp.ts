    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: nexus mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in nexuscode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .nexuscode/ subdirectory too)
  const candidates = [path.join(baseDir, "nexuscode.json"), path.join(baseDir, "nexuscode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".nexuscode", "nexuscode.json"), path.join(baseDir, ".nexuscode", "nexuscode.jsonc"))
  }
  ...
  // Default to nexuscode.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., nexus x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "nexuscode-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
