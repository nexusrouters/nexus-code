...
import type { Hooks } from "@nexus-code/plugin"
...
async function loadFreeLogin(): Promise<(() => Promise<void>) | undefined> {
  ...
  try {
    ...
    return typeof mod.nexusFreeLogin === "function" ? mod.nexusFreeLogin : undefined
  } catch {
    return undefined
  }
}
...
async function nexusLogin() {
  ...
  const nexusHook = hooks.findLast((h) => h.auth?.provider === "xiaomi")
  if (!nexusHook?.auth) {
    prompts.log.error("Nexus auth plugin not found")
    return
  }

  const method = nexusHook.auth.methods[0]
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (remaining > 0) {
      prompts.log.error(t("cli.providers.nexus_login.decrypt_retry", { remaining }))
    } else {
      prompts.log.error(t("cli.providers.nexus_login.decrypt_exhausted"))
    }
  }
}
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "nexuscode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.provider === "xiaomi") {
          await nexusLogin()
          return
        } else if ((args.provider === "nexus" || args.provider === "nexus-free") && freeLogin) {
          await freeLogin()
          return
          ...
          const choice = await prompts.select({
            ...
            options: [
              { label: "Nexus", value: "xiaomi", hint: t("cli.providers.nexus.recommended_hint") },
              ...(freeLogin
                ? [{ label: "Nexus Auto (free)", value: "nexus-free", hint: t("cli.providers.nexus_free.hint") }]
                : []),
          ...
          if (choice === "nexus-free" && freeLogin) {
            await freeLogin()
            return
          }
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in nexuscode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via nexuscode.json options (profile, region, endpoint) or\n" +
...
export const ProvidersWhoamiCommand = cmd({
  ...
  async handler(_args) {
    ...
    if (!info) {
      prompts.log.error("Not logged in. Run `nexus auth login` to log in.")
      return
    }
    if (info.type === "api" && info.metadata) {
      prompts.log.info(`Provider: Nexus`)
