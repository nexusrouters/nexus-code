...
import { Slug } from "@nexus-code/shared/util/slug"
...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in nexuscode, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's nexuscode-native continuation messages in the same session.
          ...
          // Preserve nexuscode-owned metadata on re-sync: keep any user rename
