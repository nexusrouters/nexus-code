  ...
  try {
    ...
    for (const sess of sessions) {
      ...
      try {
        ...
        // If this import previously created a nexuscode session that the user has
        ...
        // session backward past nexuscode-native activity (cc/codex parity).
        let existingUpdated: number | undefined
        if (existing) {
          const nexusSess = Database.use((db) =>
          ...
          if (!nexusSess) {
            ...
            existingUpdated = nexusSess.updated
          }
        }
