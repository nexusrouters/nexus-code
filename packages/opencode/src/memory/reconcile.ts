...
export async function indexFromDisk(
  ...
  // For CC files, derive type from frontmatter; nexus files keep loc.type from path.
  const finalType =
...
export async function reconcileMemory(
  ...
  let indexedCount = 0
  for (const p of nexusFiles) {
    const loc = parsePath(p)
    if (!loc) {
      log.warn("path outside memory layout, skipping", { path: p })
      continue
    }
    const result = await indexFromDisk(p, loc, "nexus", indexed.get(p))
