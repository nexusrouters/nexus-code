...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".nexuscode", "plugin")
...
test("opencode and opencode-go providers are disabled by MimoFreeAuthPlugin", async () => {
  ...
  expect(providers[ProviderID.make("nexus")]).toBeDefined()
  expect(providers[ProviderID.make("nexus")].models[ModelID.make("nexus-auto")]).toBeDefined()
  expect(providers[ProviderID.make("nexus")].models[ModelID.make("nexus-auto")].limit.context).toBe(1_000_000)
  expect(providers[ProviderID.make("nexus")].models[ModelID.make("nexus-auto")].limit.output).toBe(128_000)
})
