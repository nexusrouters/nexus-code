...
describe("Actor tool subagent_type enum (F36)", () => {
  ...
  it.live("flattened schema keeps operation as the sole root key (nexus can't drop the discriminator)", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const tool = yield* ActorTool
        const def = yield* tool.init()
        const fakeModel = {
          providerID: "nexus",
          api: { id: "nexus-v2.5-pro", npm: "@ai-sdk/openai-compatible" },
          id: "nexus-v2.5-pro",
          capabilities: { input: {} },
        } as any
        const flat = transformSchema(fakeModel, z.toJSONSchema(def.parameters)) as any
        // Root must expose ONLY `operation`. A flat bag (the bug) lets nexus omit
