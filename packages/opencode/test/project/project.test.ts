...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const legacy = path.join(tmp.path, ".git", "nexuscode")
    expect(await Bun.file(legacy).exists()).toBe(false)
    // v5 UUID cache file should exist.
    const idFile = path.join(tmp.path, ".git", "nexuscode-project-id")
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "nexuscode-project-id")
