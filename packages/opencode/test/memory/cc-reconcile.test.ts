  ...
  itLive.live("indexes CC files under scope='cc' with frontmatter type", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const nexusRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(nexusRoot, { recursive: true, force: true }))
...
Body about cache.`
        ...
        yield* Effect.promise(() => reconcileMemory({ nexus: nexusRoot, cc: ccBase }))
  ...
  itLive.live("union prune: deleting one CC file leaves nexus rows untouched", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const memory = yield* Memory.Service
        const nexusRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(nexusRoot, { recursive: true, force: true }))
        yield* Effect.promise(() => fs.mkdir(path.join(nexusRoot, "global"), { recursive: true }))
        yield* Effect.promise(() => fs.writeFile(path.join(nexusRoot, "global", "m.md"), "nexus body"))
...
cc body`,
        ...
        expect(rows.length).toBe(1)
        expect(rows[0].scope).toBe("global") // nexus row preserved
      }),
    ),
  )
...
body`,
        ...
        yield* Effect.promise(() => reconcileMemory({ nexus: nexusRoot }))
        expect(
          Database.use((db) =>
            db.select().from(MemoryFtsTable).where(eq(MemoryFtsTable.scope, "cc")).all(),
          ).length,
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(() =>
          reconcileMemory({ nexus: nexusRoot, cc: "/definitely/not/a/real/path/abc123" }),
        )
