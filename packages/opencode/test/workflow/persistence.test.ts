...
describe("journalKey", () => {
  ...
  test("opts key order does not change the hash", () => {
    const a = journalKey("p", { agentType: "g", model: { providerID: "nexus", modelID: "m" } }, 0)
    const b = journalKey("p", { model: { modelID: "m", providerID: "nexus" }, agentType: "g" }, 0)
    expect(a).toBe(b)
  })
  ...
  test("a string model ref hashes by its value, not its resolution", () => {
    ...
    expect(journalKey("p", { agentType: "g", model: "nexus/nexus-v2.5-pro" }, 0)).not.toBe(lite)
