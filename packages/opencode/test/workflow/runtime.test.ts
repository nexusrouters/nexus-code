...
describe("WorkflowRuntime per-agent timeout (straggler-abort)", () => {
  // A single hung agent (e.g. a persistent nexus TTFT wall) must not stall the whole
...
// the REASON to triage (nexus TTFT timeout vs spawn-reject vs over-cap). The
...
// instead of silently dropping to unbounded — which used to let a wedged nexus
