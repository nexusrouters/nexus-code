...
//
// Unlike the mock-LLM suite, this drives WorkflowRuntime against the REAL nexus
...
// the live router + a real key in ~/.config/nexuscode/nexuscode.json). Run with:
...
// The real nexus provider, read verbatim from the user's config (key included).
function realConfig() {
  const home = path.join(os.homedir(), ".config", "nexuscode", "nexuscode.json")
  const full = JSON.parse(readFileSync(home, "utf8"))
  const nexus = full.provider?.nexus
  if (!nexus?.options?.apiKey) throw new Error("no nexus provider/key in " + home)
  return { provider: { nexus } }
}

const realRef = { providerID: ProviderID.make("nexus"), modelID: ModelID.make("nexus-v2.5-pro") }
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            // Two saved workflows on disk, resolved by name from .nexuscode/workflows.
            const wfDir = path.join(dir, ".nexuscode", "workflows")
