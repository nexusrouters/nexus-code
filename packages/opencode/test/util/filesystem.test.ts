...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "nexuscode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "nexuscode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexuscode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexuscode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["nexuscode.json", "nexuscode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "nexuscode.json"),
        path.join(tmp.path, "nexuscode.jsonc"),
        path.join(project, "nexuscode.json"),
        path.join(project, "nexuscode.jsonc"),
      ])
    })
  })
